...
export default function CopilotKitPage() {
  return (
    ...
    >
      <CopilotKit
        publicApiKey={process.env.NEXT_PUBLIC_COPILOT_API_KEY as string}
      >
        <YourMainContent />
        <CopilotSidebar
...
function YourMainContent() {
  ...
  useEffect(() => {
    setMcpServers([
      // {endpoint: process.env.NEXT_PUBLIC_COPILOTKIT_SERVER_URL as string},
  ...
  const classes = {
    wrapper:
      "min-h-screen w-screen flex flex-col items-center py-10 gap-6 transition-colors duration-300",
    container:
    ...
    input:
      "bg-white/20 p-4 rounded-xl text-white relative group hover:bg-white/30 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500",
    submitButton:
      "w-full p-4 rounded-xl bg-indigo-500 text-white hover:bg-indigo-600 transition-all",
  };
    ...
    <div style={{ backgroundColor: themeColor }} className={classes.wrapper}>
      <div className={classes.container}>
        ...
        <p className="text-gray-200 text-center">
          Discover more MCP servers at{" "}
          <a href="https://mcp.composio.dev" className="text-indigo-200">
            mcp.composio.dev.
          </a>
        </p>
